...
public class SmoothOrbitCam : MonoBehaviour
{
    ...
    public void AddAngleY(float Angle,float Duration)
    {
        transform.position = new Vector3(0, 1, 0);
        transform.DORotate(new Vector3(0, transform.eulerAngles.y + Angle, 0), Duration);
        //StartCoroutine(RotateMe(Angle,Duration));
    }

    IEnumerator RotateMe(float Angle,float Duration)
    {
        float Interval = Duration / Angle;
        float TargetAngle = transform.eulerAngles.y + Angle;
        int Start = (int)transform.eulerAngles.y;

        for (int i= Start; i<= TargetAngle; i++)
        {
    ...
    {
        transform.eulerAngles = new Vector3(ValueX, ValueY, transform.eulerAngles.z);
        ResetValues();
    }
